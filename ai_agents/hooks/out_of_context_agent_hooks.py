...
class out_of_context_agent_hooks(AgentHooks[AppContext]):
    """Hooks para el Agente Fuera de Contexto."""
    async def on_end (
        self,
        ...
        print("out END of context agent hooks")
    ...
    async def on_start (
        ...
        """Se ejecuta cada vez que este agente es invocado (incluyendo después de un handoff)."""
        print("out START of context agent hooks")
        milisecond = time.perf_counter()
        debugging_obj = {
            "event": "agent_start",
            "agent_name": agent.name,
