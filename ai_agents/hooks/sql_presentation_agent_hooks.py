...
class sql_presentation_agent_hooks(AgentHooks[AppContext]):
    """Hooks para el Agente de Presentación."""
    async def on_end (
        self,
        ...
        print("sql presentation END of context agent hooks")
    ...
    async def on_start (
        ...
        """Se ejecuta cada vez que este agente es invocado (incluyendo después de un handoff)."""
        print("sql presentation START of context agent hooks")
        milisecond = time.perf_counter()
        debugging_obj = {
            "event": "agent_start",
            "agent_name": agent.name,
